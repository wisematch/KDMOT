...
def triplet_loss(embedding, targets, margin, norm_feat, hard_mining):
    ...
    if comm.get_world_size() > 1:
        ...
        #all_camfeat = concat_all_gather(cam_feat)
        all_targets = concat_all_gather(targets)
    else:
        all_embedding = embedding
        all_targets = targets
        #all_camfeat = cam_feat
    
    #new_embedding = torch.cat([all_embedding, -all_camfeat], 1)
    #new_embedding1 = torch.cat([all_embedding, 0.2 *all_camfeat], 1)
    #dist_mat =  0.8-torch.mm(new_embedding, new_embedding1.t())
